		...
		for p=0,WAR.PersonNum-1 do
			if WAR.Person[p]["死亡"]==false then
				if WAR.Person[p]["Time"]>1000 then 
					WarDrawMap(0);
					ShowScreen();
					local surid=h_lib_SaveSur(CC.ScreenW-80,0,CC.ScreenW,200);
...
function DrawTimeBar()
	...
	local xunhuan=true;
	local surid=h_lib_SaveSur(x1-(10+(x2-x1)/2),0,x2+10+20+30,y*2+18+25);
	--local surid2=h_lib_SaveSur(25,545,750,585);
	...
	while xunhuan do
		h_lib_LoadSur(surid,x1-(10+(x2-x1)/2),0)
		--h_lib_LoadSur(surid2,25,545)
		for i=0,WAR.PersonNum-1 do
	...
	lib.Delay(100);
	h_lib_FreeSur(surid);
	--h_lib_FreeSur(surid2);
end

function DrawTimeBar2()
	local x1,x2,y=CC.ScreenW*5/8,CC.ScreenW*15/16,CC.FontSmall*5;
	local draw;
	local surid=h_lib_SaveSur(x1-(10+(x2-x1)/2),0,x2+5+30,y*2+18+25);     
	while true do
		draw=false;
		for i=0,WAR.PersonNum-1 do		    
			if WAR.Person[i]["死亡"]==false then			   
...
--eft  武功动画效果id  eft.idx/grp中的效果
    ...
    for i=0,framenum-1 do
		...
		if i<fightdelay then   --只显示出招
			...
			if i==1 and WAR.Person[WAR.CurID][CC.TXDH]~=-1 then
				...
				local ssid=h_lib_SaveSur(CC.ScreenW/2-80,CC.ScreenH/2-hb-150,CC.ScreenW/2+80,CC.ScreenH/2-hb+30);
				for ii=CC.Effect[theeft],1,-1 do
					...
					ShowScreen();
					lib.Delay(30);
					h_lib_LoadSur(ssid,CC.ScreenW/2-80,CC.ScreenH/2-hb-150);
				end
				h_lib_FreeSur(ssid);
	...
	--end
	--if not fanji then
	local sssid=h_lib_SaveSur(0,0,CC.ScreenW,CC.ScreenH);
	local TP=0;
	local TPXS={};
	for i = 0, WAR.PersonNum-1 do	
...
if HitXYNum>0 then
	...
	local surid=h_lib_SaveSur(0,0,CC.ScreenW,CC.ScreenH);
	for y=3,5 do
